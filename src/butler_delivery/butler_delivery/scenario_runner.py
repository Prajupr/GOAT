...
class ScenarioRunner(Node):
    ...
    def scenario_4(self):
        """Cancel order — two cases per assessment:
        (a) Cancel while going to TABLE → robot returns to kitchen, then home
        (b) Cancel while going to KITCHEN → robot returns home directly
        """
        # ── Part A: Cancel while going to table ──
        self.get_logger().info("=== SCENARIO 4a: Cancel while going to TABLE ===")
        ...
        # Wait for robot to start heading to table
        if not self.wait_for_state("going_to_table"):
            return
        time.sleep(2.0)
        self.get_logger().info("Cancelling while en route to table...")
        self.cancel_order(-1)

        # Robot should go to kitchen first, then home
        self.wait_for_idle(timeout=180.0)
        self.get_logger().info("=== SCENARIO 4a COMPLETE ===")

        # ── Part B: Cancel while going to kitchen ──
        self.get_logger().info("=== SCENARIO 4b: Cancel while going to KITCHEN ===")
        self.publish_order([1])
        ...
        if not self.wait_for_state("going_to_kitchen"):
            return
        time.sleep(2.0)
        self.get_logger().info("Cancelling while en route to kitchen...")
        self.cancel_order(-1)

        # Robot should return home directly
        self.wait_for_idle(timeout=180.0)
        self.get_logger().info("=== SCENARIO 4b COMPLETE ===")
...
def main(args=None):
    ...
    if len(sys.argv) < 2:
        ...
        print("  4 - Cancel order: (a) while going to table, (b) while going to kitchen")
