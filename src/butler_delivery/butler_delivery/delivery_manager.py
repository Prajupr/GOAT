...
class ButlerDeliveryManager(Node):
    ...
    # Positions for the restaurant world (12m x 10m)
    # Home: near reception area
    HOME_POSITION = {"x": 0.0, "y": 0.0, "z": 0.0}
    # Kitchen: through doorway, in front of the blue pickup counter (3.5, 0.25)
    KITCHEN_POSITION = {"x": 2.5, "y": 0.2, "z": 0.0}
    # Tables — close to actual table models
    TABLE_POSITIONS = {
        1: {"x": -2.2, "y": 2.0, "z": 0.0},   # near table_1 (round, at -3.0, 2.0)
        2: {"x": -2.2, "y": -1.5, "z": 0.0},   # near table_2 (rect, at -3.0, -1.5)
        3: {"x": 0.0, "y": -1.8, "z": 0.0},    # near table_3 (round, at 0.0, -2.5)
    }
    ...
    def cancel_navigation(self):
        with self._nav_lock:
            self._nav_generation += 1  # ignore any in-flight callbacks
    ...
    def orders_callback(self, msg: Int32MultiArray):
        ...
        # Dedup: skip tables already active or pending
        active_tables = {
            o.table_id for o in self.active_orders
            if o.status not in ["canceled", "delivered", "timeout"]
        }
        pending_tables = {o.table_id for o in self.pending_orders}
        for table_id in new_tables:
            if table_id in active_tables or table_id in pending_tables:
                self.get_logger().info(
                    f"Skipping duplicate order for table {table_id}"
                )
                continue
    ...
    def _read_nav_state(self):
        """Atomically read navigation state under lock."""
        with self._nav_lock:
            return self._navigating, self._nav_result_ready, self._nav_succeeded

    def _handle_nav_failure(self, destination: str):
        """Common handler when navigation fails — abort batch and go home."""
        self.get_logger().error(f"Navigation to {destination} failed, aborting batch")
        for order in self.active_orders:
            if order.status not in ["delivered", "canceled", "timeout"]:
                order.status = "canceled"
        self.current_state = DeliveryState.RETURNING_HOME
        self.navigate_to(self.HOME_POSITION)
        self.publish_state()
        self.publish_status(f"Nav failed ({destination}), returning home")

    def handle_current_delivery(self):
        navigating, nav_result_ready, nav_succeeded = self._read_nav_state()
        # While navigating, wait for it to finish
        if navigating:
            return
        # If not navigating and no result ready, navigation hasn't started yet
        # (e.g. navigate_to() was just called but send_goal_async hasn't fired)
        # Only states that don't require nav result can proceed without it
        needs_nav_result = self.current_state in (
            DeliveryState.GOING_TO_KITCHEN,
            DeliveryState.GOING_TO_TABLE,
            DeliveryState.RETURNING_TO_KITCHEN_BEFORE_HOME,
            DeliveryState.RETURNING_HOME,
        )
        if needs_nav_result and not nav_result_ready:
            return

        if self.current_state == DeliveryState.GOING_TO_KITCHEN:
            if nav_result_ready:
                if not nav_succeeded:
                    self._handle_nav_failure("kitchen")
                    return
            ...
            if self.kitchen_ready or (
                ...
                if order.table_id in self.TABLE_POSITIONS:
                    ...
                    self.get_logger().error(
                        f"No position for table {order.table_id}, skipping"
                    )
                    order.status = "canceled"
                    self.advance_after_current_order()
            return
        ...
        if self.current_state == DeliveryState.GOING_TO_TABLE:
            if nav_result_ready:
                if not nav_succeeded:
                    self._handle_nav_failure(f"table")
                    return
                cur = self.get_current_order()
                if not cur:
                    self.current_state = DeliveryState.RETURNING_TO_KITCHEN_BEFORE_HOME
            ...
            if nav_result_ready:
                if not nav_succeeded:
                    self.get_logger().warn(
                        "Failed to reach kitchen on return, heading home directly"
                    )
                self.current_state = DeliveryState.RETURNING_HOME
                self.navigate_to(self.HOME_POSITION)
                self.publish_state()
        ...
        if self.current_state == DeliveryState.RETURNING_HOME:
            if nav_result_ready:
                if not nav_succeeded:
                    self.get_logger().warn(
                        "Failed to reach home, resetting to IDLE anyway"
                    )
